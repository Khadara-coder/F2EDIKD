...
def get_master_data() -> dict[str, Any]:
    ...
    # Load materials - support single file or split parts
    materials_files = []
    if materials_path.exists():
        materials_files = [materials_path]
    else:
        # Check for split files (part1, part2, ...)
        import glob as _glob
        parts = sorted(_glob.glob(str(MASTER_DATA_DIR / "10564_Materials_part*.csv")))
        if parts:
            materials_files = [Path(p) for p in parts]
    materials_row_count = 0
    for mat_file in materials_files:
        with open_master_csv(mat_file) as handle:
            reader = csv.DictReader(handle, delimiter=";")
            for row in reader:
                materials_row_count += 1
    ...
    data["materials_count"] = materials_row_count
    if materials_files:
        data["files"]["10564_Materials"] = {"rows": materials_row_count}
